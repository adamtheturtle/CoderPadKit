...
//  coderpad
//
//  Immutable seed data for the fake Screen API: campaigns and test sessions (in every
//  status, completed ones carrying a scored report with a per-technology/skill
//  breakdown). Keys are the snake_case the `ScreenAPI` models decode; timestamps are
//  epoch milliseconds, computed relative to now so the demo always looks recent.
//
...
nonisolated enum MockScreenFixtures {
    ...
    private nonisolated(unsafe) static let seedTests: [[String: Any]] =
        backendTests() + frontendTests() + fullStackTests() + dataScienceTests() + mapProductTests()
    ...
    /// A small Map-product cohort so `product=map` filtering is observable (#132).
    private static func mapProductTests() -> [[String: Any]] {
        [
            session(id: 5401, campaign: 101, status: "completed",
                    name: "Map Candidate", email: "map@example.com",
                    sent: 3, started: 2, ended: 2, tags: ["map"],
                    report: report(score: 70, duration: 2400,
                                   comparative: 55, warnings: [],
                                   technologies: backendTech(java: 70, algorithms: 72)),
                    product: "map"),
            session(id: 5402, campaign: 102, status: "waiting",
                    name: "Map Waiting", email: "map-wait@example.com",
                    sent: 0.4, started: nil, ended: nil, tags: [],
                    product: "map")
        ]
    }
